...
filepath = hde.filepath


def array_to_tuple_l(frame, f_no):
...
def array_to_tuple_r(frame, f_no):
...
def create_video():
    ...
    out = cv2.VideoWriter(current_time + '_' + filepath[7:-4] + '_' + mc + '_' + md + '_' + mt + '.avi', cv2.VideoWriter_fourcc(*'DIVX'), int(hde.video_fps), size)
    print('----------------------------------------------------------------------------------')
    print('Making video.......')
